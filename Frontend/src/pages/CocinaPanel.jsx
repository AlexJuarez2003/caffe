...
const CocinaPanel = () => {
  ...
  const cargarPedidos = () => {
    ...
    fetchWithAuth("http://localhost:8000/orders/by-role/")
      ...
      .catch(() => notify({ type: "error", title: "Error al cargar pedidos", duration: 3000 }))
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (actualizado.status === "ready") {
          ...
          notify({ type: "success", title: "Pedido listo para entrega", duration: 3000 });
        } else {
          setPedidos((prev) =>
        ...
        notify({ type: "error", title: "Error al actualizar pedido", duration: 3000 });
      }
    } catch {
      notify({ type: "error", title: "Error al conectarse al servidor", duration: 3000 });
    } finally {
    ...
    <div className="min-h-screen bg-[#F4F3ED] flex">
      ...
      <div className="w-64 bg-[#2D3A22] text-white p-6 flex flex-col justify-between shadow-xl">
        <div>
          <div className="mb-10 text-center">
            ...
            <p className="text-[10px] text-gray-400 tracking-widest uppercase mt-1 font-bold">Cocina</p>
          </div>
          <div className="space-y-4">
            <div className="bg-[#3D4F31] rounded-2xl p-4 text-center">
              ...
              <p className="text-4xl font-black text-amber-400 mt-1">{pendientes.length}</p>
            </div>
            <div className="bg-[#3D4F31] rounded-2xl p-4 text-center">
              <p className="text-[10px] text-gray-400 uppercase tracking-widest font-bold">En preparación</p>
              <p className="text-4xl font-black text-blue-400 mt-1">{enPreparacion.length}</p>
            </div>
          </div>
        </div>
        <button
      ...
      <div className="flex-1 p-10">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-4xl font-black text-[#2D3A22] italic">Panel de Cocina</h1>
            <p className="text-gray-500 text-sm mt-1">Gestiona los pedidos entrantes.</p>
          </div>
          <button
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-32 text-gray-300">
            ...
            <p className="font-black text-sm uppercase tracking-widest">Sin pedidos por ahora</p>
          </div>
        ) : (
            ...
            {enPreparacion.length > 0 && (
              <div className="space-y-4">
                <h2 className="text-[11px] font-black text-blue-600 uppercase tracking-[0.3em] flex items-center gap-2">
                  <ChefHat className="w-4 h-4" /> En preparación ({enPreparacion.length})
                </h2>
...
const TarjetaPedido = ({ pedido, expandido, setExpandido, actualizando, onCambiarEstado }) => {
  const config = STATUS_CONFIG[pedido.status];
  const estaExpandido = expandido === pedido.id;

  return (
    <div className={`bg-white rounded-4x1 shadow-md border-2 transition-all ${
      ...
      >
        <div className="flex justify-between items-start">
          ...
          <span className="font-black text-[#2D3A22] text-xl">${pedido.total}</span>
        </div>
          ...
          {pedido.items.map((item) => (
            <div key={item.id} className="bg-gray-50 rounded-2xl p-4 border border-gray-100">
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-[#2D3A22] text-sm">${item.subtotal}</span>
              </div>
              {item.notes && (
          ...
          {pedido.delivery_location && (
            <div className="bg-orange-50 rounded-2xl p-4 border border-orange-100">
              ...
              <p className="text-sm font-black text-[#2D3A22]">
                {pedido.delivery_location.classroom_name || pedido.delivery_location.area_name || "—"}
              </p>
