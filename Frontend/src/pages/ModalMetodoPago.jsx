...
const ModalPago = ({ isOpen, onClose, total, onConfirmar }) => {
  ...
  const handleFinalizar = async () => {
    setStatus("loading");
    const exitoso = await onConfirmar(metodoSeleccionado);
    if (exitoso) {
      setStatus("success");
      setTimeout(() => {
        setStatus("idle");
      ...
      <div className="relative bg-white w-full max-w-md rounded-[3rem] p-8 shadow-2xl overflow-hidden border border-gray-100">
        {status === "success" && (
          <div className="absolute inset-0 bg-white z-210 flex flex-col items-center justify-center p-8 text-center animate-in fade-in zoom-in duration-300">
            ...
            <h3 className="text-2xl font-black text-[#2d3a1a] italic uppercase tracking-tighter">
              ¡Pedido Confirmado!
            </h3>
            <div className="mt-4 p-4 bg-gray-50 rounded-4x1 w-full border border-gray-100">
              <p className="text-gray-400 font-bold text-[10px] uppercase tracking-widest">
                Pago registrado mediante
              </p>
              <p className="text-[#2d3a1a] font-black text-sm uppercase mt-1">
                {metodoSeleccionado}
              </p>
            </div>
        ...
        <div className="space-y-3 mb-8">
          {metodos.map((metodo) => (
            <button
              ...
              className={`w-full flex items-center justify-between p-5 rounded-4x1 border-2 transition-all text-left ${
            ...
            >
              <div className="flex items-center gap-4">
                <div
                  className={`p-3 rounded-xl shadow-sm bg-white ${metodoSeleccionado === metodo.id ? "text-orange-500" : "text-gray-400"}`}
                >
                  {metodo.icono}
                </div>
                <div>
                  <p className="font-black text-[#2d3a1a] text-sm tracking-tight">
                    {metodo.nombre}
                  </p>
